...
The mail thread stays internal to Agent Mail and is not part of the bridge contract. The provider session `threadId` returned by `claude`, `codex`, `agy`, or `copilot` remains unrelated and is consumed only by the matching `*-reply` tool. Include the target's canonical Agent Mail name, current milestone, completed work, next step, and blocker state in each checkpoint.
