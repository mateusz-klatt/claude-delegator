...
14. **Grok is the one bridged provider whose `read-only` denies** - and only because the bridge says so explicitly. `--permission-mode plan` cancelled a write and a shell escape under an insistent prompt, then was defeated by a permissive allow list in the caller's own Claude Code settings, which grok reads. The bridge therefore always adds `--deny Write/Edit/Bash`, which overrode that allow list. `--sandbox` is never emitted: `--sandbox read-only` is accepted by the CLI and did not stop a write — the same shape as Agy's `--mode plan`, a flag whose name outruns its behaviour.
    ...
    It is verified on **three hosts** (WSL, macOS and Linux) with identical verbatim denial strings, two of them placing control and test in the same directory so the written control file sat beside the absent target. Reproduction took all day because a shared free-tier limit blocked it; a SuperGrok subscription lifted the limit per account, but it propagates per host — a CLI that has not re-authenticated returns "no file" for every case *including the control*, which is exactly the result that looks like success.

    Two caveats travel with this. Project instruction files in `cwd` are auto-loaded with no off switch — `CLAUDE.md` on all four platforms, a planted `AGENTS.md` too — so delegating grok into this repository injects its own `CLAUDE.md`, the same surface as Kimi's. And **isolating `HOME` for this experiment is not safe by default, in either obvious form.** grok refreshes its token in the background — the vendor documents it, in `~/.grok/docs/user-guide/02-authentication.md`: tokens "auto-refresh silently via the stored `refresh_token`". A refresh consumes that token and writes the new pair wherever `auth.json` resolves to in the child's HOME, leaving a spent credential wherever it did not. A whole-directory symlink has no link to replace, so the new token reaches the operator's real file and auth survives — but grok's whole state set is written there too. Per-file symlinks keep the real directory clean, but rotation replaces the linked `auth.json` with a real file in the scratch HOME, and deleting that directory destroys the only valid credential: one host had to re-authenticate. A third host, whose token was minutes old, had nothing to rotate and saw neither effect. Copying rather than linking does not help, since rotation invalidates the old token wherever a copy sits. Either skip the isolation, or copy `auth.json` back before removing the scratch directory. None of this invalidates a permission result, which comes from the permission layer rather than from state.
